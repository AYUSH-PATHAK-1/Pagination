    ...
    <>
      <div className="container mx-auto mt-12 border-black p-5 border-[2px] rounded-xl">
        <table className="min-w-full bg-white border border-gray-300">
          <thead>
            <tr>
              <th className="py-2 px-4 border-b text-center border-black border-[1px] border-solid">
                ID
              </th>
          ...
          <tbody>
            {records.map((d, i) => (
              <tr key={i} className={i % 2 === 0 ? "bg-green-100" : "bg-white"}>
                <td className="py-2 px-4 border-b text-center align-middle border-black border-[1px] border-solid">
                  {d.id}
                </td>
                <td className="py-2 px-4 border-b text-center align-middle border-black border-[1px] border-solid">
                  {d.name}
          ...
          <ul className="flex list-none p-2 border-black border-[1px] rounded-lg">
            ...
            {numbers
              .slice(
                ...
                Math.min(curruntpage + 3, npage)
              )
              .map((n, i) => (
                <li
                  key={i}
                  className={`mr-3 ${
                    curruntpage === n ? "bg-blue-500" : ""
                  } p-2 rounded-lg`}>
                  <a
                    href="#"
                    className="page-link"
